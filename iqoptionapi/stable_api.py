...
class IQ_Option:
    ...
    def get_all_open_time(self, type_to_get='turbo'):        #LRH: 'all': all | 'binaries': turbo and binary | 'other': cfd, forex, crypto
        ...
        elif 'binary' in type_to_get:
            binary_lisr = ["binary"]
        elif 'turbo' in type_to_get:
            binary_list = ["turbo"]
        else:
            pass_towards = False
        ...
        # for digital
        if 'digital' in type_to_get or 'all' in type_to_get:
            pass_towards = True
        else:
            pass_towards = False
        ...
        # for OTHER
        if 'other' in type_to_get or 'all' in type_to_get:
            pass_towards = True
        else:
            pass_towards = False
    ...
    def get_all_profit(self, type_to_get):        #LRH: 'turbo' | 'binary' | 'all' or 'binaries'
        all_profit = nested_dict(2, dict)
        init_info = self.get_all_init()
        
        if 'binaries' in type_to_get:
            type_to_get = ['all']

        if 'turbo' in type_to_get or 'all' in type_to_get:
            for actives in init_info["result"]["turbo"]["actives"]:
                name = init_info["result"]["turbo"]["actives"][actives]["name"]
                name = name[name.index(".") + 1:len(name)]
        ...
        if 'binary' in type_to_get or 'all' in type_to_get:
            for actives in init_info["result"]["binary"]["actives"]:
                name = init_info["result"]["binary"]["actives"][actives]["name"]
                name = name[name.index(".") + 1:len(name)]
    ...
    def check_win_v4(self, id_number):
        ...
        try:
            if self.get_async_order(id_number)["option-closed"] != {}:
                value = self.get_async_order(id_number)["option-closed"]["msg"]["profit_amount"] - \
                    self.get_async_order(id_number)["option-closed"]["msg"]["amount"]
            else:
                success = False
        except:
